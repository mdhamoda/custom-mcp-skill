...
worked examples: [`assets/mcp-server/`](assets/mcp-server/) — a small 3-tool server
(`AccountWithOpportunity`) covering `aa:`/`fa:`/`psmcps:` side by side, plus a full 17-tool combined
server (`SalesRevOpsExample`) covering every backing type including `nq:`, scrubbed/renamed from a
real org deployment. **`../../deployable-examples/salesRevOpsMcp/`** (sibling to `skills/`, at the
repo root) is the same 17-tool server packaged as a real, standalone, `sf project deploy`-able SFDX
project with its own `sfdx-project.json` and a deploy-order README — for actually standing it up in
an org, not just reading as reference.
