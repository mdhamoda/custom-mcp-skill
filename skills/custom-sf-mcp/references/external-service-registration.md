...
```
node .claude/skills/custom-sf-mcp/scripts/esr-toolkit.mjs esr aura <ClassName> [--org <alias>] [--deploy]
node .claude/skills/custom-sf-mcp/scripts/esr-toolkit.mjs esr apexrest <ClassName> --cls-file <path> [--org <alias>] [--deploy]
node .claude/skills/custom-sf-mcp/scripts/esr-toolkit.mjs esr namedquery <QueryApiName> --soql "<SOQL>" --label "<label>" --description "<desc>" [--param name:label:description ...] [--org <alias>] [--deploy]
```
